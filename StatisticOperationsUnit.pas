...
Type
    ...
    class function FilterStatistic(Var StatisticArr : TStatisticArr;FilterMethod : TFilterMethod):TStatisticArr;static;
    ...
    class function FilterToday(Statistic : TTypeStatistic) : Boolean;static;
    end;
...
class function TStatisticOperations.FilterToday(Statistic : TTypeStatistic) : Boolean;
Var
    IsToday : Boolean;
    I : Integer;
Begin
    I := DayOf(Statistic.Date);
    IsToday := DayOf(Statistic.Date) = DayOf(Now);
    IsToday := (IsToday) And ( MonthOf(Statistic.Date) = MonthOf(Now));
    IsToday :=  (IsToday) And ( YearOf(Statistic.Date) = YearOf(Now));
    FilterToday := IsToday;
End;

class function TStatisticOperations.FilterStatistic(Var StatisticArr : TStatisticArr;FilterMethod : TFilterMethod):TStatisticArr;
Var
...
Begin
    ...
    Begin
        if (FilterMethod(StatisticArr[I])) then
        Begin
